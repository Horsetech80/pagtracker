...
export interface CheckoutConfigData {
  ...
  /** ID do tenant (obrigatório para EntityService) */
  tenant_id: string;

  /** ID do usuário que criou (obrigatório para EntityService) */
  user_id: string;
  ...
  /** Data de criação (obrigatório para EntityService) */
  created_at: string;

  /** Data de atualização (obrigatório para EntityService) */
  updated_at: string;
} 